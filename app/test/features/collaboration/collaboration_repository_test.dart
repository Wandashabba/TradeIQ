...
void main() {
  ...
  test('listAnnouncements GETs /announcements and parses the list', () async {
    final adapter = _CapturingAdapter(
      '[{"id":"a1","title":"Q3 Kickoff","body":"New targets are live",'
      '"createdAt":"2026-07-09T10:00:00.000Z"}]',
    );
    final previous = dio.httpClientAdapter;
    dio.httpClientAdapter = adapter;
    ...
    final announcements =
        await DioCollaborationRepository().listAnnouncements();

    expect(adapter.captured!.method, 'GET');
    expect(adapter.captured!.path, '/announcements');
    expect(announcements.single.title, 'Q3 Kickoff');
  });
}
