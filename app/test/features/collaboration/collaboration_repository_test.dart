...
void main() {
  ...
  test('listAnnouncements GETs /announcements and parses the envelope',
      () async {
    final adapter = _CapturingAdapter(
      '{"data":[{"id":"a1","title":"Q3 Kickoff","body":"New targets are live",'
      '"createdAt":"2026-07-09T10:00:00.000Z"}],"nextCursor":"cursor-1"}',
    );
    final previous = dio.httpClientAdapter;
    dio.httpClientAdapter = adapter;
    addTearDown(() => dio.httpClientAdapter = previous);
  ...
  test('listMessages GETs /messages and parses the envelope', () async {
    final adapter = _CapturingAdapter(
      '{"data":[{"id":"m1","senderId":"s1","recipientId":"r1",'
    ...
    expect(page.data.single.body, 'Hello team');
    expect(page.nextCursor, 'cursor-1');
  });
}
