  ...
  'data': [
    {
      ...
      'lastPing': {
        ...
        'lng': 28.05,
        'accuracyM': 8,
        'recordedAt': '2026-09-15T08:29:20.000Z',
      },
      'ageSeconds': 45,
    ...
    },
    {
      ...
      'lastPing': {
        ...
        'lng': 28.06,
        'accuracyM': null,
        'recordedAt': '2026-09-15T08:28:05.000Z',
      },
      'ageSeconds': 120,
      'currentOutlet': null,
      'lastOutlet': {
    ...
    },
    {
      ...
      'lastPing': {
        'lat': -26.14,
        'lng': 28.07,
        'recordedAt': '2026-09-15T08:18:05.000Z',
      },
      'ageSeconds': 720,
      'currentOutlet': null,
      'lastOutlet': null,
    },
    {
      ...
      'lastPing': {
        'lat': -26.16,
        'lng': 28.08,
        'recordedAt': '2026-09-15T06:25:05.000Z',
      },
      'ageSeconds': 7500,
      'currentOutlet': null,
      'lastOutlet': null,
    ...
    },
    {
      ...
      'lastPing': {
        ...
        'lng': 28.09,
        'accuracyM': 180,
        'recordedAt': '2026-09-15T08:29:05.000Z',
      },
      'ageSeconds': 60,
      'currentOutlet': null,
      'lastOutlet': {
...
void main() {
  ...
  group('AgentLocationsPage.fromJson', () {
    ...
    test(
      'a declined agent is never pinned, even if a position slipped through',
      () {
        final json = _json();
  ...
  test('descriptions lead with age and name where the place came from', () {
    expect(
      liveAgentDescription(page.agents[0]),
      '45s old · At store · Thabo · at Sandton Spar',
    );
    expect(
      liveAgentDescription(page.agents[1]),
      '2 min old · In transit · Lerato · last check-in Rosebank PnP',
    );
    expect(
      liveAgentDescription(page.agents[4]),
      'never shared · Offline · Pieter',
    );
    expect(
      liveAgentDescription(page.agents[5]),
      '1 min old · Near store · Ayanda · near Sandton Spar',
    );
    // Not sharing has no position to age, so the state leads.
    expect(
      liveAgentDescription(page.agents[6]),
      'Not sharing · Zanele · last check-in Rosebank PnP',
    );
  });
  ...
  test('the six live states differ by shape, not colour (#144)', () {
    final types = {
      for (final s in LiveAgentState.values) livePainterTypeFor(s),
    };
    ...
    expect(
      livePainterTypeFor(LiveAgentState.nearStore),
      isNot(livePainterTypeFor(LiveAgentState.atStore)),
    );
    expect(
      livePainterTypeFor(LiveAgentState.nearStore),
      isNot(livePainterTypeFor(LiveAgentState.inTransit)),
    );
    // Not sharing is not offline.
    expect(
      livePainterTypeFor(LiveAgentState.notSharing),
      isNot(livePainterTypeFor(LiveAgentState.offline)),
    );
  });
  ...
  for (final (name, theme) in [
    ...
    for (final onDark in [false, true]) {
      testWidgets(
        'the legend names every state by glyph and word ($name theme, onDark: $onDark)',
        (tester) async {
          await tester.pumpWidget(
            routedApp(
              Scaffold(body: LiveStateLegend(onDark: onDark)),
              theme: theme,
            ),
          );
          ...
          for (final state in LiveAgentState.values) {
            ...
            expect(
              find.descendant(
                of: entry,
                matching: find.text(liveStateLabel(state)),
              ),
              findsOneWidget,
            );
            final glyph = tester.widget<LiveAgentStateGlyph>(
              find.descendant(
                of: entry,
                matching: find.byType(LiveAgentStateGlyph),
              ),
            );
            expect(glyph.state, state);
            final paint = tester.widget<CustomPaint>(
              find
                  .descendant(of: entry, matching: find.byType(CustomPaint))
                  .first,
            );
  ...
  group('LiveLocationsSection', () {
    ...
    for (final (name, theme) in [
      ...
      testWidgets(
        'shows last updated, the legend, and rows age first ($name theme)',
        (tester) async {
          await tester.pumpWidget(section(theme: theme));
          await tester.pumpAndSettle();

          expect(
            find.byKey(const ValueKey('live-last-updated')),
            findsOneWidget,
          );
          expect(
            find.text('Last updated ${formatUpdatedAt(_serverTime)}'),
            findsOneWidget,
          );
          for (final state in LiveAgentState.values) {
            expect(
              find.byKey(ValueKey('live-legend-${state.name}')),
              findsOneWidget,
            );
          }
          for (final label in [
          ...
          for (final (id, age) in [
            ...
            final label = tester
                .getSemantics(find.byKey(ValueKey('live-row-$id')))
                .label;
      ...
      testWidgets(
        'rows for near store and not sharing carry glyph, word and place ($name theme)',
        (tester) async {
          await tester.pumpWidget(section(theme: theme));
          await tester.pumpAndSettle();
          ...
          expect(
            find.descendant(of: near, matching: find.text('1 min')),
            findsOneWidget,
          );
          expect(
            find.descendant(of: near, matching: find.text('Near store')),
            findsOneWidget,
          );
          expect(
            find.descendant(
              of: near,
              matching: find.text('Ayanda · near Sandton Spar'),
            ),
            findsOneWidget,
          );
          expect(
            tester
                .widget<LiveAgentStateGlyph>(
                  find.descendant(
                    of: near,
                    matching: find.byType(LiveAgentStateGlyph),
                  ),
          ...
          expect(
            find.descendant(of: declined, matching: find.text('—')),
            findsOneWidget,
          );
          expect(
            find.descendant(of: declined, matching: find.text('Not sharing')),
            findsOneWidget,
          );
          expect(
            find.descendant(
              of: declined,
              matching: find.text('Zanele · last check-in Rosebank PnP'),
            ),
            findsOneWidget,
          );
          expect(
    ...
    testWidgets('keeps the last update on screen when a poll fails', (
      tester,
    ) async {
  ...
  testWidgets('polls on the interval, and stops when nothing is watching', (
    tester,
  ) async {
    ...
    final container = ProviderContainer(
      overrides: [
        ...
        liveLocationsPollIntervalProvider.overrideWithValue(
  ...
  for (final (name, theme) in [
    ...
    testWidgets(
      'the dashboard panel draws live pins with age-first labels ($name theme)',
      (tester) async {
        ...
        expect(
          find.byKey(const ValueKey('live-agent-pin-a-never')),
          findsNothing,
        );
        ...
        expect(
          find.descendant(
            of: nearPin,
            matching: find.text('1 min · Near Sandton Spar'),
          ),
          findsOneWidget,
        );
        expect(
        ...
        expect(
          tester
              .widget<Semantics>(
                find
                    .descendant(of: nearPin, matching: find.byType(Semantics))
                    .first,
              )
              .properties
              .label,
        ...
        expect(
          find.byKey(const ValueKey('live-agent-pin-a-declined')),
          findsNothing,
        );
        expect(
          find.byKey(const ValueKey('live-row-a-declined')),
          findsOneWidget,
        );
        expect(tester.takeException(), isNull);
  ...
  testWidgets(
    'today’s trail map carries the live layer and says when it was updated',
    (tester) async {
      ...
      expect(
        find.byKey(const ValueKey('live-agent-pin-a-transit')),
        findsOneWidget,
      );
      // The T0 trail stays.
      expect(
        find.byKey(const ValueKey('agent-stop-a-store-0')),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets(
    'the trail map shows live positions even before anyone checks in',
    (tester) async {
      await tester.pumpWidget(
        routedApp(
          const AgentTrailScreen(),
      ...
      expect(
        find.byKey(const ValueKey('live-agent-pin-a-store')),
        findsOneWidget,
      );
    },
  );
