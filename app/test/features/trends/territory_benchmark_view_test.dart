...
TerritoryBenchmarkReport _report(
  ...
  targetLabel: metric == BenchmarkMetric.scorecards ? 'Green threshold' : null,
  unassignedCount: 1,
...
void main() {
  ...
  testWidgets('says so when the client has no territories', (tester) async {
    await _openCompare(
      tester,
      _FakeRepo(onBenchmark: (m) async => _report(m, territories: const [])),
      theme: AppTheme.dark(),
    );
