...
void main() {
  group('trend table', () {
    ...
    test(
      'aligns the two series by position when they differ in length',
      () async {
    ...
    test(
      'skips a row it cannot read rather than plotting it as zero',
      () async {
        final table = artifactTableFor(
          detail(
            type: 'trend_chart',
  ...
  group('pillar table', () {
    ...
    test(
      'labels a metric it has never heard of instead of hiding it',
      () async {
        final table = artifactTableFor(
          detail(type: 'pillar_metrics', data: const {'newFangledScore': 7}),
        )!;
  ...
  test(
    'a spec with no meaningful table says so, rather than inventing one',
    () {
      // A table of pin coordinates is not the "table-view twin" the design
      // system means by the term — it is filler.
      expect(
        artifactTableFor(detail(type: 'outlet_map', data: const {})),
        isNull,
      );
      expect(
