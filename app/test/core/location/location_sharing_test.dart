...
class _Harness {
  _Harness(this.container, this.db, this.repo, this.store, this.location, this.clock);
  ...
  LocationSharingState get state => container.read(locationSharingControllerProvider);
  ...
  Future<List<SyncQueueItem>> rows(WidgetTester tester, String type) async =>
      (await tester.runAsync(() => (db.select(db.syncQueueItems)
...
Future<_Harness> _start(
  ...
  final container = ProviderContainer(
    overrides: [
      ...
      syncServiceProvider.overrideWithValue(SyncService(db: db, flusher: _NoopFlusher())),
    ],
  );
  addTearDown(() async {
...
void _heartbeatTest(String description, Future<void> Function(WidgetTester tester) body) {
...
void main() {
  ...
  _heartbeatTest('the notice gates pings: nothing is sent until the agent acknowledges', (tester) async {
    ...
    expect(h.store.saved['agent-1']?.decision?.consent, LocationConsent.acknowledged);
    // The first ping goes straight away, then one per interval.
    expect(await h.rows(tester, locationPingEntity), hasLength(1));
  });

  _heartbeatTest('queues one ping per server-set interval through the outbox', (tester) async {
    final h = await _start(tester, settings: _settings(consent: LocationConsent.acknowledged, interval: 300));
    expect(h.state.running, isTrue);
    expect(await h.rows(tester, locationPingEntity), hasLength(1));
  ...
  _heartbeatTest('pauses in the background and resumes in the foreground', (tester) async {
    final h = await _start(tester, settings: _settings(consent: LocationConsent.acknowledged));
    expect(h.state.running, isTrue);
  ...
  _heartbeatTest('a refused or missing location is not an error: nothing queues, noFix is set', (tester) async {
    final h = await _start(tester, settings: _settings(consent: LocationConsent.acknowledged));
    expect(await h.rows(tester, locationPingEntity), hasLength(1));
  ...
  _heartbeatTest('declining stops the timer and drops pings that had not sent', (tester) async {
    final h = await _start(tester, settings: _settings(consent: LocationConsent.acknowledged));
    expect(await h.rows(tester, locationPingEntity), hasLength(1));
  ...
  _heartbeatTest('an answer given offline is kept over a server that has not heard it yet', (tester) async {
    final store = _Store()..saved['agent-1'] = _settings(consent: LocationConsent.acknowledged);
  ...
  _heartbeatTest('does nothing at all for a manager', (tester) async {
    final h = await _start(tester, settings: _settings(consent: LocationConsent.acknowledged), role: 'manager');
    expect(h.state.isAgent, isFalse);
