...
- [ ] Failing tests: `WorklistRow` renders as a white card (surface1 ground, `line` hairline, `radiusPanel`, **no shadow** — AMENDED 2026-07-25: every consumer nests rows inside a shadowed `PanelCard`, and `design/alerts.html` renders rows flat; one shadow per panel, so row cards are flat-in-panel with `boxShadow == null` asserted) with the severity left edge INSIDE the card radius (clipped); optional new `thumb:` widget slot renders leading at 44×44 rounded-8 when provided, absent otherwise; a new `WorklistCascade` (or param on existing lists' build helper) wraps rows in `OneShotEntrance` with `Motion.stagger` per index, capped at the first 12 rows (13+ render instantly), one-shot latched, reduceMotion → static. Dark theme: card uses tokens, contrast-assert title/meta on the rendered ground.
