...
**`visit_progress.dart` / `visit_review.dart`** (both `StreamProvider.family<T, String>` keyed by `visitDraftId`, with no `outletId` in scope): **Revised during implementation** — the first approach tried was an async `visitDraftOutletIdProvider` DB lookup (querying `VisitDrafts.outletId` for the given `visitDraftId`), watched before conditionally watching `skusListProvider(outletId)`. Code-quality review caught a real correctness bug in that approach: while either async layer was loading, `skuCount`/`skuNames` transiently read as empty via the `orElse` fallback, making a genuinely-partial section briefly report as "done" — and `audit_shell_screen.dart` wires `progress.canSubmit` straight to the submit button. Both real call sites (`audit_shell_screen.dart`'s hub watch and `_openSubmitGate` → `SubmitGateScreen`) turned out to already have `outletId` available synchronously, so the DB lookup was solving a problem neither caller actually had.

The shipped fix instead converts `visitProgressProvider`/`visitReviewProvider` to `StreamProvider.family<T, ({String visitDraftId, String outletId})>` — a compound record key, matching the existing `scorecards_repository.dart` precedent (`FutureProvider.family<VisitOutcome, ({String visitDraftId, String outletId})>`). `skuCount`/`skuNames` become a direct single-stage `ref.watch(skusListProvider(key.outletId))`, no async lookup anywhere in the chain. `SubmitGateScreen` gains `required this.outletId`; `audit_shell_screen.dart`'s two call sites pass `widget.outletId` and `outlet.id` respectively, both already synchronously available. `visitDraftOutletIdProvider` was removed entirely (never shipped).
